...
const { mrTemplates, mrTemplatesMsk } = require('./constants');
...
const templateShouldBeEnforcedMsk = async () => {
  // size increased of a file > created
  // size decreased of a file > deleted
  // size is same but modified > modified
  ...
  if (csv.modified) {
    ...
    csvModified.forEach(async file => {
      ...
      if (action) {
        ...
        warn(`You should use appropriate MR [${templateWithoutExt} template](${link}), provide details and a jira ticket...`);
      }
    });
  }
...
const templateShouldBeEnforced = async (files, templates) => {
  ...
  if (Object.keys(template).length === 1) {
    Object.entries(template).forEach(([key, value]) => {
      ...
      const link = `https://gitlab.com/${repo}/-/blob/master/.gitlab/merge_request_templates/${sanitized}`;
      warn(`You should use appropriate MR [${mrTemplateWithoutExt} template](${link}), provide details and a jira ticket...`);
    });
  ...
  if (csv.modified) {
    ...
    csvModified.forEach(async file => {
      ...
      if (result.length > 0) {
        // console.log(result)
        let msg = [
          "You should put the topics in alphabetical order.",
...
const welcomeMsg = input => {
  if (input) {
    markdown(`Hey there! Thanks for contributing a PR to a repo! 🎉. [ci job url](${input.url})`)
  } else {
