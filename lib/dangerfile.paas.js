  ...
  if (tfvars.modified || tfvars.created || tfvars.deleted) {
    ...
    if (tfvars.created) {
      ...
      tfvarsCreated.forEach(async file => {
        ...
        message(`📂 ${file}. ✏️  Make sure to review CI job output attribute "valid_upgrade_targets" in order to understand where the right "engine" version is configured`);
      }, Error())
    }
    if (tfvars.modified) {
...
const ensureRdsAuroraCreationValidated = async (files) => {
  ...
  if (tfvars.modified || tfvars.created || tfvars.deleted) {
    ...
    if (tfvars.modified || tfvars.created) {
      message('🤖 Make sure to verify the `plan` job in the merge request pipeline and compare "engine_info" output attribute with desired engine version .');
    }
