...
// helpers & utils
const
  { contains, hclToJson, getHashDifference, uniqueArraySize } = require("./utils");
const {
  repo, prId, links,
    ...
    if (tfvars.modified) {
      ...
      await tfvarsModified.forEach(async file => {
        ...
        if (before !== after && !infoMessages.has('instance_classes')) {
          ...
          message(`🤖 Instance class modified. Worth to provide: link to datadog dashobard, monitor, capacity planning calculations...`);
        }
      }, Error())
    }
  }
...
const templateShouldBeEnforcedMsk = async () => {
  ...
  message(`❗ For consistency, the recommended topic name pattern is \`(<vertical>-<team>|<team>)-<project|product>-<name>\` [which vertical the team is](${links.orgStructure})...`)
}
...
const addManualApplyMessage = () => {
  console.log('in: addManualAppplyMessage');
...
const changelogUnreleased = async () => {
  ...
  if (chg.modified && isForReview) {
    ...
    if (false && changes.length > 1 && firstEl.includes('[Unreleased]') && ["Added", "Changed", "Fixed", "Removed"].some(el => secondEl.includes(el))) {
      message('🤖 Found modified CHANGELOG 🎖🎖🎖.')
