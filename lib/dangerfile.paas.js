    ...
    if (tfvars.created) {
      ...
      match(tfvarsCreated, ['**/dev/**'], {}).forEach(async file => {
        ...
        if (engine === 'mysql' && !match.isMatch(instance_class, rdsRecommendInstanceTypesInDev)) {
          warn(`📂 ${file}. ➡️  (💸 saving) In \`dev\` environment instance class \`${instance_class}\` not recommended. Consider different sizes \`${rdsRecommendInstanceTypesInDev}\` ...`);
        }
      }, Error());
      ...
      tfvarsCreated.forEach(async file => {
        const diff = await danger.git.diffForFile(file);
        const data = hclToJson(diff.after);
        let { engine, engine_version, family, storage_type } = data.rds_config.instance_config
        ...
        if (arrayContainsString(['mysql', 'postgres'], engine) && storage_type in recommendedRDSStorageTypes) {
          ...
          warn(`📂 ${file}. ✏️  Recommended \`storage_type\` is \`${recommended}\` as it provides better performance and cost efficiency as opposite to  \'${storage_type}\'. [Docs and Migration AWS migration Guide](${links.gp2gp3Migration}).`)
        }
      }, Error())
    }
    if (tfvars.modified) {
      ...
      await tfvarsModified.forEach(async file => {
        ...
        if (before !== after && !infoMessages.has('instance_classes')) {
          ...
          message(`🤖 Instance class modified. Worth to provide: link to datadog dashboard, monitor, capacity planning calculations...`);
        }
        if (arrayContainsString(['mysql', 'postgres'], engine) && storage_type in recommendedRDSStorageTypes) {
          const recommended = recommendedRDSStorageTypes[storage_type];
...
const templateShouldBeEnforced = async (templates) => {
  ...
  if (Object.keys(template).length === 1) {
    ...
    fail(`multiple resources 'created|modified|deleted' for resources ${template} in a single MR.`)
  }
}
...
const changelogUnreleased = async () => {
  ...
  if (chg.modified && isForReview) {
    ...
    } else if (["Added", "Changed", "Fixed", "Removed"].some(el => diff.diff.includes(el))) {
