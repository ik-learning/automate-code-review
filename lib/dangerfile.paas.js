...
const skipReview = () => {
  ...
  if (!result && danger.gitlab.commits.length > 0) {
    let diff = Math.abs(new Date(danger.gitlab.mr.created_at) - new Date(danger.gitlab.commits[0].created_at));
    let minutes = Math.floor((diff / 1000) / 60);
    const thresholdInMinutes = 5;
    if (minutes > thresholdInMinutes && reviewOnce) {
      console.log(`skip MR review as last commit is older then ${thresholdInMinutes}.`);
      result = true
    }
  }
  if (!result) {
