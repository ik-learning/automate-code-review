  ...
  if (csv.modified) {
    ...
    csvModified.forEach(async file => {
      ...
      if (action) {
        ...
        warn(`Please use the appropriate MR [${templateWithoutExt} template](${link}), provide details and a jira ticket...`);
      }
    });
  }
...
const templateShouldBeEnforced = async (files, templates) => {
  ...
  if (Object.keys(template).length === 1) {
    Object.entries(template).forEach(([key, value]) => {
      ...
      const link = `https://gitlab.com/${repo}/-/blob/master/.gitlab/merge_request_templates/${sanitized}`;
      warn(`Please use the appropriate MR [${mrTemplateWithoutExt} template](${link}), provide details and a jira ticket...`);
    });
